...
def create_app(test_config=None):
  ...
  def delete_question(question_id):
    ...
    select_question = Question.query.filter(Question.id == question_id).one_or_none()
  ...
  def get_quiz_question():
    ...
    try:
      ...
      question_query = Question.query.filter(Question.category == quiz_category['id']).filter(Question.id.notin_((previous_questions))).all()

      new_question = question_query[random.randrange(0, len(question_query),1)] if len(question_query) >0 else None
